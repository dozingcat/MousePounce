...
class _EmpHomePageState extends State<EmpHomePage> {
  ...
  bool _aiHasMoodForPile(final List<PileCard> pileCards) {
    ...
    for (PileCard pc in pileCards) {
      int cval = moodWeights[pc.card.rank] ?? 1;
      total += cval;
    }
    return total > 16;
  }
    ...
    setState(() {
      ...
      switch (game.rules.badSlapPenalty) {
        ...
        case BadSlapPenaltyType.slap_timeout:
          ...
          break;
        default:
          throw AssertionError('Unknown penalty type: ${game.rules.badSlapPenalty}');
      }
    });
